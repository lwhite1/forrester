...
class XmileExporterTest {
    ...
    class RoundTrip {
        ...
        void shouldRoundTripStocksAndFlows() {
            ...
            assertThat(imported.constants()).hasSize(1);
            assertThat(imported.constants().get(0).name()).isEqualTo("rate");
            assertThat(imported.constants().get(0).value()).isEqualTo(0.02);
        }
