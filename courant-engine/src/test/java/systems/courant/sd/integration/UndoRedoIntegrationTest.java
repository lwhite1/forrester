...
class UndoRedoIntegrationTest {
    ...
    class SnapshotStack {
        ...
        void shouldSupportRedo() {
            ...
            assertThat(undone.parameters()).isEmpty();

            // Redo to v1
            ModelDefinition redone = serializer.fromJson(snap1);
            assertThat(redone.parameters()).hasSize(1);
            assertThat(redone.parameters().getFirst().name()).isEqualTo("k");
            assertThat(redone.parameters().getFirst().literalValue()).isEqualTo(42.0);
        }
