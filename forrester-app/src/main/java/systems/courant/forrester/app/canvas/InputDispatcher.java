    ...
    void handleMouseDragged(MouseEvent event, ModelCanvas canvas) {
        ...
        if (resizeController.isActive()) {
            resizeController.drag(
                    viewport.toWorldX(event.getX()),
                    viewport.toWorldY(event.getY()),
                    canvasState, () -> canvas.saveUndoState("Resize"));
            canvas.requestRedraw();
            event.consume();
            return;
        }
        ...
        if (dragController.isDragging()) {
            dragController.drag(event.getX(), event.getY(),
                    canvasState, viewport, () -> canvas.saveUndoState("Move"));
            canvas.requestRedraw();
            event.consume();
        }
    }
    ...
    void handleMouseReleased(MouseEvent event, ModelCanvas canvas) {
        ...
        if (reattachController.isActive()) {
            ...
            if (dx * dx + dy * dy < MIN_REATTACH_DRAG_SQ) {
                ...
                reattachController.complete(
                        viewport.toWorldX(event.getX()),
                        viewport.toWorldY(event.getY()),
                        canvasState, editor, () -> canvas.saveUndoState("Reconnect flow"));
        ...
        if (rerouteController.isActive()) {
            boolean rerouted = rerouteController.complete(
                    viewport.toWorldX(event.getX()),
                    viewport.toWorldY(event.getY()),
                    canvasState, editor, () -> canvas.saveUndoState("Reroute connection"));
            if (rerouted) {
                canvas.clearSelectedConnection();
                canvas.regenerateConnectors();
            }
    ...
    void handleKeyPressed(KeyEvent event, ModelCanvas canvas) {
        ...
        if (event.getCode() == KeyCode.ESCAPE) {
            ...
            switch (event.getCode()) {
                ...
                case DIGIT5 -> { canvas.switchTool(CanvasToolBar.Tool.PLACE_CONSTANT); event.consume(); }
                case DIGIT6 -> { canvas.switchTool(CanvasToolBar.Tool.PLACE_MODULE); event.consume(); }
                case DIGIT7 -> { canvas.switchTool(CanvasToolBar.Tool.PLACE_LOOKUP); event.consume(); }
                case DIGIT8 -> { canvas.switchTool(CanvasToolBar.Tool.PLACE_CLD_VARIABLE); event.consume(); }
                case DIGIT9 -> { canvas.switchTool(CanvasToolBar.Tool.PLACE_CAUSAL_LINK); event.consume(); }
                default -> { }
            }
        }
    }
