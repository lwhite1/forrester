...
class DashboardPanelFxTest {
    ...
    @Test
    @DisplayName("Clicking variable label in chart legend invokes onVariableClicked callback")
    void clickingVariableLabelInvokesCallback(FxRobot robot) {
        AtomicReference<String> clicked = new AtomicReference<>();
        Platform.runLater(() -> {
            panel.setOnVariableClicked(clicked::set);
            panel.showSimulationResult(dummyResult());
        });
        WaitForAsyncUtils.waitForFxEvents();

        // Switch to Chart tab inside the SimulationResultPane
        Platform.runLater(() -> {
        ...
        robot.clickOn("#seriesLabel-Stock1");
        WaitForAsyncUtils.waitForFxEvents();
