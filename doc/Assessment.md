...
- The visual editor covers core modeling operations comprehensively — element creation/deletion, inline editing, flow connections, resize, undo/redo, connection hover/selection/deletion, copy/paste, and diagram export (PNG, JPEG, SVG). Remaining gaps compared to commercial tools are minor: no connection re-routing via drag (only delete and re-create), no cut (Ctrl+X), and no cross-model paste. These are polish-level gaps rather than architectural gaps — the foundations (rendering, interaction, serialization, simulation integration) are solid.
...
| Non-programmers | Good — visual editor enables diagram-based modeling with a properties panel, context toolbar, integrated charting, feedback loop highlighting, connection interaction/deletion, copy/paste, multi-format diagram export (PNG/JPEG/SVG), hover feedback, and element resizing |
...
1. **Cut (Ctrl+X)** — Copy/paste exists but cut (copy + delete in one action) is a natural complement that users expect.
2. **Connection re-routing** — Connections can be deleted but not dragged to a different target. Adding drag-to-reroute for info links would complete the connection editing story.
3. **Lookup table editor** — LookupTableDef exists in the engine but has no UI for creating/editing graphical function tables. A table/chart editor in the properties panel would unlock a major SD modeling pattern.
