...
- Interactive visual editor closes the biggest learning gap. The `forrester-app` module provides a JavaFX canvas-based editor for creating and editing stock-and-flow diagrams interactively. Users can create stocks, flows, auxiliaries, constants, and modules via toolbar or keyboard shortcuts (1–6), connect flows to stocks with a two-click protocol, edit names/values/equations inline with double-click, drag elements to reposition, resize elements via corner handles, and reattach flow endpoints by dragging clouds onto stocks. A properties panel shows editable fields for the selected element (name, value, equation, unit, negative-value policy) with a context toolbar offering rename, delete, drill-into, and bindings actions. Info link connections support hover highlighting (thickened line on mouse-over) and click-to-select (dashed highlight with connection properties shown in the panel), with Escape to deselect — giving connections the same interactive feedback as elements. The editor includes pan (Space+drag, middle/right-drag), zoom (scroll wheel, Ctrl+Plus/Minus/0), rubber-band marquee selection, undo/redo (Ctrl+Z/Shift+Z, 100-level snapshot stack), hover highlighting (subtle outline on mouse-over for discoverability), context-sensitive cursor feedback for all interaction states, and a status bar showing tool/selection/element counts/zoom. Models are saved to and loaded from JSON files with full view layout preservation. Simulation can be run directly from the editor (Ctrl+R) with results displayed in a tabbed dialog offering both a sortable table and an interactive line chart with per-series toggle checkboxes and PNG export. Diagrams can be exported to PNG or JPEG images (File → Export Diagram, Ctrl+E) at 2× resolution for crisp rendering in documents and presentations — the export renders the full diagram to an offscreen canvas, framing the world-space bounding box with padding, and suppresses interactive overlays (selection, hover, rubber-band) while preserving loop highlights if active. A "Loops" toggle button highlights feedback loop structures — elements and edges participating in cycles are drawn with colored borders and thickened lines, and the status bar shows the loop group count. The editor renders the Layered Flow Diagram visual language with distinct shapes for each element type (including module containers with "mod" badge), material flow arrows routed through diamond indicators, dashed info link connectors, and cloud symbols for disconnected endpoints.
...
- The visual editor covers core modeling operations comprehensively — element creation, inline editing, flow connections, resize, undo/redo, connection hover/selection, and diagram image export. Remaining gaps compared to commercial tools are minor: no connection deletion or re-routing via the UI, no SVG export, and no copy/paste. These are refinement gaps rather than architectural gaps — the foundations (rendering, interaction, serialization, simulation integration) are solid.
...
| Non-programmers | Good — visual editor enables diagram-based modeling with a properties panel, context toolbar, integrated charting, feedback loop highlighting, connection interaction, diagram export, hover feedback, and element resizing |
...
1. **Connection deletion** — Click-selected connections can't yet be deleted or re-routed. Adding Delete key support for the selected connection (with undo) would complete the connection interaction story.
2. **Copy/paste** — Ctrl+C/V for duplicating elements or groups within or across models would speed up diagram construction.
3. **SVG export** — PNG/JPEG export is available; SVG would enable lossless scaling and editing in vector tools like Inkscape or Illustrator.
