...
| Production/enterprise modeling | Fair — has analysis tools but lacks visual diagrams and arrays/subscripts |
...
- It's not a replacement for Vensim or Stella for serious modeling work. The absence of
  visual diagrams and arrays/subscripts means analysts would hit walls on models that require visual feedback-loop analysis or multi-cohort structures.
...
2. **Arrays/subscripts** — Important for real-world models (age cohorts, regions, product lines) but a large design effort. Lower priority than the analysis features above because the current workaround (duplicate stocks) is ugly but functional for small models.
